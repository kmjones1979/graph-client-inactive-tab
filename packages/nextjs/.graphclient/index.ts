...
export type Burn_orderBy =
  ...
  | 'pool__totalValueLockedETHUntracked'
...
export type MyQueryQuery = { transactions: Array<{ swaps: Array<Maybe<(
      Pick<Swap, 'amount0' | 'amount1' | 'amountUSD'>
      & { pool: { token1: Pick<Token, 'name'>, token0: Pick<Token, 'name'> } }
    )>> }> };
...
export function getSdk<C, E>(requester: Requester<C, E>) {
  return {
    MyQuery(variables?: MyQueryQueryVariables, options?: C): Promise<MyQueryQuery> {
      return requester<MyQueryQuery, MyQueryQueryVariables>(MyQueryDocument, variables, options) as Promise<MyQueryQuery>;
    }
  };
}
